...
function EditarPosion({ data }) {
    ...
    function enviarDatos(nombre, imagen, cantidad, precio, descripcion, categoria) {
        ...
        fetch(`http://localhost:3000/posiones/edit/${data.id}`, {
            method: "PUT",
            headers: {
    ...
    return (
        <div className="w-full h-full p-8 xl:p-16 bg-black rounded-xl border-[3px] border-indigo-400">
            <form className="w-full h-full text-start"
                onSubmit={e => {
                    e.preventDefault()
                    enviarDatos(nombre, imagen, cantidad, precio, descripcion, categoria);
                    Context.cambio.setCambio("a")
                }}>
                <h1 className="text-indigo-400 text-4xl text-center mb-12">Editar Posion</h1>
